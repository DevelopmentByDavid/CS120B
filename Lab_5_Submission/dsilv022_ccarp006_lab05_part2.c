...
enum States{START, INIT, LED1, LED2, LED3, WAIT, WAIT2} state;
...
void Tick(){
	...
	case INIT: state = LED1;
		break;
	case LED1: state = button ? WAIT : LED2;
		break;
	case LED2: state =  button ? WAIT : LED3;
		break;
	...
	switch (state)
	{
		...
		case INIT: PORTB = 0x00;
			break;
		case LED1: PORTB =  PORTB = 0x01;
			break;
		case LED2:PORTB =  PORTB = 0x02;
			break;
